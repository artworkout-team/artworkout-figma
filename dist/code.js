...
function lintStep(page, step) {
    ...
    assert(!ss || ss >= 20 || maxSize <= 100, `Should not use ss<20 with long lines. Consider using bg template. ${ss}<20 ${maxSize}>100`, page, step, ErrorLevel.INFO);
    assert(!ss || ss >= 20 || terminalNodes.length <= 8, `Should not use ss<20 with too many lines. Consider using bg template. ${ss}<20 ${terminalNodes.length}>8`, page, step, ErrorLevel.INFO);
