...
function lintFills(node, page, fills) {
    ...
    fills.forEach((f) => {
        ...
        if (f.type === 'IMAGE') {
            assert(f.opacity == 1 || !rgbt, 'Image fill must not be opaque', page, node, ErrorLevel.WARN);
        }
    });
}
function lintStrokes(node, page, strokes) {
    ...
    strokes.forEach((s) => {
        ...
        if (s.type === 'IMAGE') {
            assert(s.opacity == 1 || !rgbt, 'Image stroke must be opaque', page, node, ErrorLevel.WARN);
        }
    });
...
function lintVector(page, node) {
    ...
    assert(!rgbt || !!anim, "Must have 'blink' or 'draw-line'", page, node, ErrorLevel.WARN); // every rgbt must have animation
}
