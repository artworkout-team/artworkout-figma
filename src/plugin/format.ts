...
function formatOrder(lesson: FrameNode) {
  ...
  const steps = lesson.findChildren(
    (n) =>
      getTags(n).includes('step') && !getTags(n).includes('s-multistep-result')
  )
  const result = lesson.findChild((n) =>
  ...
  let sortedSteps = steps.sort(
    (a, b) =>
      JSON.parse(b.getPluginData('layer')) -
...
function autoFormat() {
  const thumbPage = figma.root.children.find(
    (p) => p.name.toUpperCase() == 'THUMBNAILS'
  )
  if (thumbPage) {
  ...
  findAll(figma.root, (node) => /^settings/.test(node.name)).forEach(
    (n: GroupNode) => {
      n.resize(40, 40)
      n.x = 10
      n.y = 10
  ...
  findAll(figma.root, (node) =>
    ...
    if (n.children[0].type === 'RECTANGLE') {
      ...
      const nChildren = n.children[0] as unknown as GroupNode
